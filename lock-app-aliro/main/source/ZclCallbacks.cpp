...
{
    bool bResult = LockManager::Instance().SetCredential(endpointId, credentialIndex, creator, modifier, credentialStatus, credentialType,
                                                         credentialData);
    if (bResult)
    {
        // Handle ALIRO credential types 
        switch (credentialType)
        {
            ...
            case CredentialTypeEnum::kAliroNonEvictableEndpointKey:
                bResult = NxpAliroDelegate::Instance().SetAliroCredential(endpointId, credentialIndex, creator, modifier,
...
void emberAfDoorLockClusterInitCallback(EndpointId endpoint)
{
    ...
    // Set Aliro delegate to the NXP implementation, must be done after LockManager initialization
    // a default delegate class is set in the InitEndpoint method
    DoorLockServer::Instance().SetDelegate(endpoint, &NxpAliroDelegate::Instance());
