...
| `thread_ftd_se05x_default_crypto_with_cli_gpio` | Thread Full Thread Device (FTD) configuration and SE05X default crypto CLI enabled with GPIO control for SE05X power on/off and to get the GPIO notification. Refer [NXP SE05X Guide](../../../middleware/matter/docs/platforms/nxp/nxp_se05x_guide.md) for connections details. | frdmmcxw72@cm33_core0 | `prj_thread_ftd.conf` | `-DCONFIG_MCUX_COMPONENT_middleware.freertos-kernel.config=n`<br>`-DCONFIG_CHIP_SE05X=y`<br>`-DCONFIG_CHIP_LIB_SHELL=y`<br>`-DCONFIG_SE05X_HOST_GPIO=y` |
| `thread_ftd_se05x_scp03_default_crypto_with_cli_gpio` | Thread Full Thread Device (FTD) configuration and SE05X default crypto with scp03 session enabled. CLI enabled with GPIO control for SE05X power on/off and to get the GPIO notification. Refer [NXP SE05X Guide](../../../middleware/matter/docs/platforms/nxp/nxp_se05x_guide.md) for connections details. | frdmmcxw72@cm33_core0 | `prj_thread_ftd.conf` | `-DCONFIG_MCUX_COMPONENT_middleware.freertos-kernel.config=n`<br>`-DCONFIG_CHIP_SE05X=y`<br>`-DCONFIG_CHIP_LIB_SHELL=y`<br>`-DCONFIG_SE05X_HOST_GPIO=y`<br>`-DCONFIG_SE05X_SCP03=y` |
| `thread_ftd_se05x_aes_key_default_crypto_with_cli_gpio` | Thread Full Thread Device (FTD) configuration and SE05X default crypto with aes key session enabled. CLI enabled with GPIO control for SE05X power on/off and to get the GPIO notification. Refer [NXP SE05X Guide](../../../middleware/matter/docs/platforms/nxp/nxp_se05x_guide.md) for connections details. | frdmmcxw72@cm33_core0 | `prj_thread_ftd.conf` | `-DCONFIG_MCUX_COMPONENT_middleware.freertos-kernel.config=n`<br>`-DCONFIG_CHIP_SE05X=y`<br>`-DCONFIG_CHIP_LIB_SHELL=y`<br>`-DCONFIG_SE05X_HOST_GPIO=y`<br>`-DCONFIG_SE05X_AES_KEY=y` |
