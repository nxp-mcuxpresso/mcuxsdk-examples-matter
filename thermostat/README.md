...
| `se05x_wifi_iw610_default_crypto_gpio` | Wi-Fi configuration with IW610 transceiver and SE05X secure element (Node Oper key in SE05X) with host GPIO control for SE05X power on/off. | evkcmimxrt1060 | `prj_wifi.conf` | `-DCONFIG_MCUX_COMPONENT_component.wifi_bt_module.IW61X=y`<br>`-DCONFIG_MCUX_COMPONENT_component.wifi_bt_module.board_murata_2ll_m2=y`<br>`-DCONFIG_CHIP_SE05X=y`<br>`-DCONFIG_SE05X_HOST_GPIO=y` |
| `se05x_wifi_default_crypto_gpio` | Wi-Fi configuration with SE05X secure element (Node Oper key in SE05X) along with GPIO control for SE05X power on/off. | frdmrw612 | `prj_wifi.conf` | `-DCONFIG_CHIP_SE05X=y`<br>`-DCONFIG_SE05X_HOST_GPIO=y` |
